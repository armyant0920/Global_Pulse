...
const GlobalPulse = (function () {
  ...
  async function renderIndex() {
    ...
    document.getElementById('filters').hidden = reports.length <= 1;
  ...
  async function renderReport() {
    ...
    // Build item-level region + topic filter bar
    const itemRegions = [...new Set(items.flatMap(i =>
      i.region ? [i.region] : (i.regions || [])
    ))].sort();
    const itemTopics = [...new Set(items.flatMap(i => i.topic_tags || []))].sort();

    const reportFiltersEl = document.getElementById('report-filters');
    const bar = document.createElement('div');
    bar.className = 'item-filter-bar';
    bar.innerHTML =
      (itemRegions.length ? `
        <label class="ifbar-label"><span>地區</span>
          <select id="if-region">
            <option value="">全部地區</option>
            ${itemRegions.map(r => `<option value="${escapeHTML(r)}">${escapeHTML(r)}</option>`).join('')}
          </select>
        </label>` : '') +
      (itemTopics.length ? `
        <label class="ifbar-label"><span>議題</span>
          <select id="if-topic">
            <option value="">全部議題</option>
            ${itemTopics.map(t => `<option value="${escapeHTML(t)}">${escapeHTML(t)}</option>`).join('')}
          </select>
        </label>` : '') +
      `<button id="if-reset" type="button">重置</button>
       <span class="ifbar-stats" id="ifbar-stats"></span>`;
    reportFiltersEl.insertAdjacentElement('afterend', bar);

    // Filter state
    let activeType = 'all';
    const regionSel = document.getElementById('if-region');
    const topicSel  = document.getElementById('if-topic');
    const statsEl   = document.getElementById('ifbar-stats');

    function applyFilters() {
      let filtered = items;

      // Type / importance filter (from chips)
      if (activeType === 'major')      filtered = filtered.filter(i => i.importance === 'major');
      else if (activeType === 'comparison') filtered = filtered.filter(i => i.type === 'comparison');
      else if (activeType === 'wildcard')   filtered = filtered.filter(i => i.type === 'wildcard' || i.importance === 'wildcard');

      // Region filter
      const region = regionSel ? regionSel.value : '';
      if (region) {
        filtered = filtered.filter(i =>
          (i.region && i.region === region) ||
          (i.regions && i.regions.includes(region))
        );
      }
      ...
      if (topic) {
        filtered = filtered.filter(i => (i.topic_tags || []).includes(topic));
      }

      statsEl.textContent = filtered.length < items.length
        ? `顯示 ${filtered.length} / ${items.length} 則`
        : '';
    ...
    if (topicSel)  topicSel.addEventListener('change', applyFilters);

    // Reset button
    document.getElementById('if-reset').addEventListener('click', () => {
