...
### Step 4：自動合 PR 進 main
push 完成後，Claude Code UI 通常會自動為這個 session 分支開啟一個 PR。請執行以下步驟把它合進 main，讓 github.io 自動部署：

1. 用 `mcp__github__list_pull_requests` 找出本 session 分支對應的開啟中 PR（head 為當前分支名）
2. 用 `mcp__github__merge_pull_request` 直接合進 main，merge method 用 `squash`，commit title 沿用 commit message
3. 合完即可結束 session，不需要等待或回報

若 PR 還沒被自動建立，改用 `mcp__github__create_pull_request` 先開一個（title/body 用 commit message），再合掉。

整個流程是「commit → push → 開 PR（若需要）→ 合 PR → 結束」，全程不要等待使用者確認。
...
GitHub Pages 只在推 `main` 時觸發部署。Routine 跑在 Claude Code session 分支上，push 後若不合 PR，網站內容會停在上一次手動合的版本（曾發生過五次 routine 結果停留在 `claude/practical-cannon-*` 分支沒合進去，github.io 卡了一個月）。

自動合 PR 的風險：壞資料會直接上線。但 JSON schema 穩定、SKILL.md 對輸出格式有約束，實務上很少出問題。萬一壞了，`git revert <commit>` 一次就回滾。

若之後要加保護，可在 repo 加一個 GitHub Actions 驗證 `website/data/` 下所有 JSON 合法、必填欄位齊全，配合 GitHub Auto-merge 設定為「CI 過才合」。
